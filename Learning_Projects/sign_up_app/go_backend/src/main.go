...
func lastPerson() {
	var n, k, lastPerson int
	fmt.Print("Enter the number of people in the queue (n): ")
	fmt.Scan(&n)
	fmt.Print("Enter the number of people who get tickets in each round (k): ")
	fmt.Scan(&k)
	var div = n / k
	var rem = n % k
	if div%2 == 0 {
		lastPerson = (div/2)*k + rem
	} else {
		lastPerson = (div/2)*k + k
	}
	fmt.Printf("Last Person to get Ticket will be %d.\n", lastPerson)
}
func main() {
	lastPerson()
	// upload()
